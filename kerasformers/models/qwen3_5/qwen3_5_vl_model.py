...
class Qwen3_5VisionModel(layers.Layer):
    """Dense Qwen3.5 vision tower: learned pos-embeds -> GELU blocks -> 2x2 merger.

    The Qwen3-VL ViT **without DeepStack** (identical to the Qwen3.5-MoE tower): a
    per-patch embedding, learned position embeddings bilinearly interpolated to each
    image's grid and added in, ``depth`` full-attention GELU blocks with 2D rotary
    positions and a block-diagonal (per-image) mask, then a single 2x2 merger to
    ``out_hidden_size``.
...
@keras.saving.register_keras_serializable(package="kerasformers")
class Qwen3_5TextModel(layers.Layer):
    """Dense Qwen3.5 hybrid decoder (no LM head), driven by external M-RoPE tables.

    ``token_embedding -> num_layers x Qwen3_5DecoderLayer -> final RMSNorm``. Layers
    alternate Gated-DeltaNet linear attention and gated full attention (every
    ``full_attention_interval``-th), each with a dense GeGLU MLP. The interleaved
    M-RoPE ``cos`` / ``sin`` tables are computed by the multimodal model and passed in
    (only the full-attention layers use them); the DeltaNet ``pad_mask`` is threaded so
...
@keras.saving.register_keras_serializable(package="kerasformers")
class Qwen3_5VLModel(Qwen2VLModel):
    """Dense Qwen3.5 multimodal backbone: ViT + dense hybrid decoder (no LM head).

    Reuses :class:`Qwen2VLModel`'s multimodal fusion (image/video placeholder scatter,
    M-RoPE position indexing via ``get_rope_index``, causal masking) with a Qwen3.5 ViT
    (no DeepStack) and the **dense** Qwen3.5 hybrid text decoder (Gated-DeltaNet + gated
    full attention, dense GeGLU MLP), using interleaved partial-rotary M-RoPE. Returns
    raw features; use :class:`Qwen3_5ConditionalGenerate` for logits / text.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # keras init via SubclassedBaseModel (same pattern as Qwen3_5MoeModel).
    ...
    def _forward_features(self, inputs):
        ...
        inputs_embeds, position_ids, _, _ = self._prepare_inputs(
            input_ids,
        ...
        attn_mask = self._causal_mask(seq, seq, offset=0, attention_mask=attention_mask)
    ...
    def config_from_hf(cls, hf_config):
        ...
        return {
            ...
            "embed_dim": hidden,
            "mlp_dim": tc.get("intermediate_size", 17408),
...
@keras.saving.register_keras_serializable(package="kerasformers")
class Qwen3_5ConditionalGenerate(Qwen3_5VLModel, BaseGeneration):
    """Dense Qwen3.5 VLM with an LM head + fast ``.generate()`` (image+text -> text).

    ``build_cache`` runs the vision encoder + interleaved-M-RoPE prefill into a **hybrid
    per-layer cache** (fixed-slot ``(key, value)`` for the full-attention layers,
    ``(conv_state, recurrent_state)`` for the Gated-DeltaNet layers), with ``rope_deltas``
    carried alongside; ``call_with_cache`` then decodes one text token at M-RoPE position
