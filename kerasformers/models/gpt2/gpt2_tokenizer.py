...
DEFAULT_TOKENIZER_REPO = "openai-community/gpt2"
...
class GPT2Tokenizer(BaseTokenizer):
    ...
    Loads the model's ``tokenizer.json`` and exposes ``encode`` / ``decode`` and a
    ``call`` that tokenizes text(s) into padded ``{"input_ids", "attention_mask"}``
    ready for ``model.generate``. GPT-2 is a base LM with no chat template.
    """
