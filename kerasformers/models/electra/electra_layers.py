...
class ElectraEmbeddings(layers.Layer):
    """Constructs ELECTRA's input embeddings.

    Sums learned word, absolute-position, and token-type (segment) embeddings,
    then applies LayerNorm and dropout. Unlike BERT the embeddings live at
    ``embedding_size`` (which may be smaller than the hidden size); the encoder's
    :func:`electra_backbone` projects them up to the hidden size afterwards.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.word_embeddings = layers.Embedding(
            vocab_size, embedding_size, name="word_embeddings"
        )
        self.position_embeddings = layers.Embedding(
            max_position_embeddings, embedding_size, name="position_embeddings"
        )
        self.token_type_embeddings = layers.Embedding(
            type_vocab_size, embedding_size, name="token_type_embeddings"
        )
...
@keras.saving.register_keras_serializable(package="kerasformers")
class ElectraSelfAttention(layers.Layer):
    """ELECTRA multi-head self-attention (the ``attention.self`` sub-block).

    Projects the input to query/key/value, computes scaled dot-product attention
    with an additive padding mask, and returns the concatenated context. The
    subsequent output projection, residual, and LayerNorm live in the encoder
    layer (matching Hugging Face's ``attention.output``).

    Args:
        embed_dim: Hidden size. Must be divisible by ``num_heads``.
...
@keras.saving.register_keras_serializable(package="kerasformers")
class ElectraFlattenChoices(layers.Layer):
...
@keras.saving.register_keras_serializable(package="kerasformers")
class ElectraUnflattenChoices(layers.Layer):
    """Inverse of :class:`ElectraFlattenChoices` for the scores: ``(B*C, 1) -> (B, C)``.
