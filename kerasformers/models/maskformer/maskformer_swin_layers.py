...
class MaskFormerSwinPatchEmbeddings(layers.Layer):
    """Conv2d 4x4 stride 4 patch projection + per-token LN."""
...
class MaskFormerSwinPatchMerging(layers.Layer):
    """2x spatial downsampling: gather 2x2 → concat last → LN → Linear /2."""
...
class MaskFormerSwinSelfAttention(layers.Layer):
    """Window self-attention with separate query/key/value and relative pos bias."""
...
class MaskFormerSwinBlock(layers.Layer):
    """Single Swin block: pre-LN → window/shifted attn → residual → LN → MLP → residual."""
...
class MaskFormerSwinStage(layers.Layer):
    """One Swin stage: list of blocks (alternating shift), optional downsample."""
