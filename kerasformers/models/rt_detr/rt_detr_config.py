...
class RTDetrConfig(BaseConfig):
    r"""Configuration for [`RTDETRDetect`], the RT-DETR real-time detector.
    ...
    ```python
    >>> from kerasformers.models.rt_detr import RTDetrConfig, RTDETRDetect

    >>> configuration = RTDetrConfig()
    >>> model = RTDETRDetect(configuration)
