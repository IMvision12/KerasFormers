...
@keras.saving.register_keras_serializable(package="kerasformers")
class GlmMLP(layers.Layer):
    """SwiGLU with a fused gate/up projection: ``down(up * silu(gate))``.

    ``gate_up`` projects to ``2 * mlp_dim`` and is split into the gate and up
    halves (bias-free), matching GLM's packed ``gate_up_proj``.
    """
...
@keras.saving.register_keras_serializable(package="kerasformers")
class GlmAttention(layers.Layer):
    """GLM grouped-query attention with partial interleaved rope.

    The ``query`` / ``key`` / ``value`` projections carry an optional bias
    (``attention_bias``); ``output_proj`` is bias-free. Rotary embeddings rotate
    only the first ``rotary_dim`` channels of each head (the partial-rope slice).
...
@keras.saving.register_keras_serializable(package="kerasformers")
class GlmDecoderLayer(layers.Layer):
    """One GLM block: pre-norm attention then pre-norm fused-SwiGLU MLP.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.attention_bias = attention_bias
        self.input_layernorm = GlmRMSNorm(eps=norm_eps, name="input_layernorm")
        ...
        self.mlp = GlmMLP(embed_dim, mlp_dim, name="mlp")
