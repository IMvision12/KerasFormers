...
DEFAULT_TOKENIZER_REPO = "mistralai/Mistral-7B-Instruct-v0.3"
...
class MistralTokenizer(BaseTokenizer):
    ...
    ``{"input_ids", "attention_mask"}`` with the ``<s>`` bos prepended,
    ...
    def apply_chat_template(self, messages, add_generation_prompt=True, system=None):
        # Mistral instruct format: each user turn is "[INST] ... [/INST]", the
        # assistant reply follows and closes with </s>; a system prompt is
        # folded into the first user turn separated by a blank line.
