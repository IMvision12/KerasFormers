...
class Glm4vMoeProcessor(BaseProcessor):
    """Image + text -> model inputs for GLM-4.5V.

    Composes the tokenizer and the (shared GLM-4V) image processor; renders the
    GLM chat template, expands each ``<|image|>`` placeholder to the right number
    of merged vision tokens, and tokenizes.
    """

    TOKENIZER_CLS = Glm4vMoeTokenizer
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.tokenizer = tokenizer or Glm4vMoeTokenizer(hf_id=hf_id)
