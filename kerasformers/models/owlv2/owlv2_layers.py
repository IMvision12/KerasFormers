...
class Owlv2PositionEmbedding(layers.Embedding):
...
@keras.saving.register_keras_serializable(package="kerasformers")
class Owlv2VisionEmbeddings(layers.Layer):
    """Patch embedding + class token + learned position embedding for OWLv2.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.position_embedding = Owlv2PositionEmbedding(
            self.num_positions,
            hidden_size,
...
@keras.saving.register_keras_serializable(package="kerasformers")
class Owlv2TextEmbeddings(layers.Layer):
    """Token embedding + learned position embedding for the OWLv2 text tower.
...
@keras.saving.register_keras_serializable(package="kerasformers")
class Owlv2Attention(layers.Layer):
...
@keras.saving.register_keras_serializable(package="kerasformers")
class Owlv2SplitBatchQueries(layers.Layer):
    """Split a flat ``(B*Q, ...)`` tensor into a per-image ``(B, Q, ...)`` tensor.

    OWLv2 receives text queries flattened across the batch (``B * Q``
    rows). This layer reshapes that flat tensor back into ``(B, Q, ...)``
    using a separate batch-reference input to recover ``B`` at runtime,
    so it works correctly under the Keras Functional API where the
    leading ``B*Q`` dim is unknown at trace time.
