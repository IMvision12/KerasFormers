...
from .deepseek_v2_config import DEEPSEEK_V2_CONFIG, DEEPSEEK_V2_WEIGHTS_URLS
from .deepseek_v2_layers import (
...
class DeepseekV2Model(SubclassedBaseModel):
    """DeepSeek-V2 MoE decoder (Multi-head Latent Attention + DeepSeekMoE).

    MLA compresses keys/values into ``kv_lora_rank`` latents with a single
    shared interleaved-rope key; the MoE layers route with a float32 softmax
    ...
    Args:
        ...
        topk_method / n_group / topk_group / routed_scaling_factor: Routing.
    ...
    BASE_MODEL_CONFIG = DEEPSEEK_V2_CONFIG
    BASE_WEIGHT_CONFIG = DEEPSEEK_V2_WEIGHTS_URLS
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.final_norm = DeepseekV2RMSNorm(eps=norm_eps, name="final_norm")
    ...
    def config_from_hf(cls, hf_config):
        return {
            ...
            "num_experts": hf_config.get("n_routed_experts", 64),
            "num_experts_per_tok": hf_config.get("num_experts_per_tok", 6),
            "n_shared_experts": hf_config.get("n_shared_experts", 2),
            "topk_method": hf_config.get("topk_method", "greedy"),
    ...
    def transfer_from_hf(cls, keras_model, hf_state_dict):
        from .convert_deepseek_v2_hf_to_keras import transfer_deepseek_v2_weights

        transfer_deepseek_v2_weights(keras_model, hf_state_dict)
...
class DeepseekV2Generate(DeepseekV2Model, BaseGeneration):
    ...
    # DeepSeek <|end_of_sentence|> (100001). Explicit generate() args override.
    eos_token_id = (100001,)
