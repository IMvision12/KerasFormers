...
class DeepseekV2RMSNorm(layers.Layer):
...
@keras.saving.register_keras_serializable(package="kerasformers")
class DeepseekV2MLP(layers.Layer):
...
@keras.saving.register_keras_serializable(package="kerasformers")
class DeepseekV2Experts(layers.Layer):
...
@keras.saving.register_keras_serializable(package="kerasformers")
class DeepseekV2MoE(layers.Layer):
    """DeepSeekMoE block: fp32 softmax router + shared experts.

    ``topk_method="greedy"`` takes a plain top-k of the softmax scores;
    ``"group_limited_greedy"`` first keeps the ``topk_group`` expert groups
    with the highest per-group max score, zeroes the rest, then takes the
    top-k. The selected softmax weights are NOT renormalized: just scaled by
    ``routed_scaling_factor``, and a shared-expert SwiGLU of the block input
    is added.
    ...
    Args:
        ...
        n_group / topk_group: Group-limited routing geometry.
        routed_scaling_factor: Routed-weight multiplier.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.experts = DeepseekV2Experts(
    ...
    def call(self, hidden_states):
        ...
        scores = ops.softmax(logits, axis=-1)  # (T, E) fp32
        if self.topk_method == "group_limited_greedy":
            grouped = ops.reshape(
                scores, (-1, self.n_group, self.num_experts // self.n_group)
            )
            group_scores = ops.max(grouped, axis=-1)  # (T, G)
            _, group_idx = ops.top_k(group_scores, self.topk_group)
            group_mask = ops.max(
                ops.one_hot(group_idx, self.n_group, dtype="float32"), axis=1
            )  # (T, G)
            score_mask = ops.repeat(
                group_mask, self.num_experts // self.n_group, axis=-1
            )
            scores = scores * score_mask
        top_vals, top_idx = ops.top_k(scores, self.num_experts_per_tok)
...
@keras.saving.register_keras_serializable(package="kerasformers")
class DeepseekV2Attention(layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if q_lora_rank is None:
            ...
            self.query_a_norm = DeepseekV2RMSNorm(eps=norm_eps, name="query_a_norm")
        ...
        self.kv_a_norm = DeepseekV2RMSNorm(eps=norm_eps, name="kv_a_norm")
...
@keras.saving.register_keras_serializable(package="kerasformers")
class DeepseekV2DecoderLayer(layers.Layer):
    """One DeepSeek-V2 block: pre-norm MLA, then pre-norm dense MLP or MoE.

    Args:
        embed_dim / num_heads + MLA dims: see :class:`DeepseekV2Attention`.
        ...
        topk_method / n_group / topk_group / routed_scaling_factor: MoE shape.
        softmax_scale: Attention score scale.
        norm_eps: RMSNorm epsilon.
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.attention_norm = DeepseekV2RMSNorm(eps=norm_eps, name="attention_norm")
        ...
        self.mlp_norm = DeepseekV2RMSNorm(eps=norm_eps, name="mlp_norm")
        if use_moe:
            ...
            self.mlp = DeepseekV2MLP(embed_dim, mlp_dim, name="mlp")
