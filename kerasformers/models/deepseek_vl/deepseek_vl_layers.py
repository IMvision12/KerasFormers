...
class DeepseekVLTextRMSNorm(layers.Layer):
...
@keras.saving.register_keras_serializable(package="kerasformers")
class DeepseekVLTextMLP(layers.Layer):
...
@keras.saving.register_keras_serializable(package="kerasformers")
class DeepseekVLTextAttention(layers.Layer):
    """DeepseekVLText grouped-query causal self-attention.
    ...
    key. When ``num_kv_heads < num_heads`` the K/V heads are repeated to match
    the query heads (the released checkpoints are plain MHA). A KV cache can be
...
@keras.saving.register_keras_serializable(package="kerasformers")
class DeepseekVLTextDecoderLayer(layers.Layer):
    """One DeepseekVLText transformer block: pre-norm attention, then pre-norm SwiGLU.
    ...
    Call args:
        hidden_states, cos, sin, attention_mask, past_key_value, use_cache: as
            in :class:`DeepseekVLTextAttention`.
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.attention_norm = DeepseekVLTextRMSNorm(eps=norm_eps, name="attention_norm")
        self.attention = DeepseekVLTextAttention(
            embed_dim, num_heads, num_kv_heads, head_dim, name="attention"
        )
        self.mlp_norm = DeepseekVLTextRMSNorm(eps=norm_eps, name="mlp_norm")
        self.mlp = DeepseekVLTextMLP(embed_dim, mlp_dim, name="mlp")
...
@keras.saving.register_keras_serializable(package="kerasformers")
class DeepseekVLVisionAttention(layers.Layer):
...
@keras.saving.register_keras_serializable(package="kerasformers")
class DeepseekVLVisionLayer(layers.Layer):
    """One SigLIP encoder block: pre-LN attention + pre-LN exact-gelu MLP.
    ...
    def __init__(self, embed_dim, mlp_dim, num_heads, norm_eps=1e-6, **kwargs):
        ...
        self.attention = DeepseekVLVisionAttention(
