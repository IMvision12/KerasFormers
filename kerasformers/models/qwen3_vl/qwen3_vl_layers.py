"""Qwen3-VL layers — deltas vs Qwen2.x-VL.

Text: Qwen3 attention with per-head QK-norm and no qkv bias (RoPE applied after
the norm), SwiGLU MLP (reused). Vision: LayerNorm blocks with a non-gated GELU
MLP (``gelu_pytorch_tanh``), and two patch-merger flavours (pre- vs
post-shuffle LayerNorm) — the post-shuffle ones produce DeepStack features.
"""
...
class Qwen3VLTextAttention(layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.q_norm = Qwen2VLRMSNorm(eps=rms_norm_eps, name="q_norm")
        self.k_norm = Qwen2VLRMSNorm(eps=rms_norm_eps, name="k_norm")
    ...
    def call(
        ...
        # (b, s, heads, head_dim) -> q_norm/k_norm over head_dim -> (b, heads, s, head_dim)
        q = self.q_norm(
            ops.reshape(
...
class Qwen3VLTextDecoderLayer(layers.Layer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.mlp = Qwen2VLMLP(
            hidden_size, intermediate_size, use_bias=False, name="mlp"
        )
