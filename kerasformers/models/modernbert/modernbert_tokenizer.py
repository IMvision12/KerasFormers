...
class ModernBertTokenizer(BaseTokenizer):
    """ModernBERT byte-level BPE tokenizer (``tokenizers`` Rust backend).

    Loads the HuggingFace fast-tokenizer ``tokenizer.json`` for ``variant`` from the
    ``kerasformers/<variant>`` repo (or an explicit ``tokenizer_file``): the byte-level
    BPE model, the ``[CLS] A [SEP]`` / ``[CLS] A [SEP] B [SEP]`` template
    post-processing, and truncation / padding are baked into the file. ``call``
    returns the ``input_ids`` / ``attention_mask`` dict expected by
    :class:`ModernBertModel`. ModernBERT has no token-type embeddings, so no
    ``token_type_ids`` are produced.

    Args:
        variant: ModernBERT variant key (default ``"modernbert_base"``).
        tokenizer_file: Optional explicit ``tokenizer.json`` path (overrides variant).
        max_seq_len: Truncation length (default 8192); batches pad to the longest.
        unk_token / sep_token / pad_token / cls_token / mask_token: Special tokens.
    """

    DEFAULT_VARIANT = "modernbert_base"
