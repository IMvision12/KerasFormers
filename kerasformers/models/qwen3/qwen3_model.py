...
class Qwen3Model(SubclassedBaseModel):
    ...
    rather than as a static graph. Returns raw features; use :class:`Qwen3TextGenerate`
    for logits / text.
    ...
    Args:
        ...
        tie_embeddings: Whether :class:`Qwen3TextGenerate` ties the LM head to the
...
@keras.saving.register_keras_serializable(package="kerasformers")
class Qwen3TextGenerate(Qwen3Model, BaseGeneration):
    ...
    decode step). Constructor ``Args`` are inherited from :class:`Qwen3Model`.
        ...
        gen = Qwen3TextGenerate.from_weights("kerasformers/qwen3-0.6b")
