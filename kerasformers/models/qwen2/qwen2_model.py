...
``.generate()``.
    ...
    gen = Qwen2Generate.from_weights("hf:Qwen/Qwen2-0.5B-Instruct")
"""
...
from .config import QWEN2_CONFIG
...
class Qwen2Model(BaseModel):
    """Qwen2 decoder: embed_tokens -> N decoder layers -> RMSNorm.

    Call with ``{"input_ids": (B, L), "attention_mask": (B, L) optional}`` ->
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.norm = Qwen2RMSNorm(eps=rms_norm_eps, name="norm")
    ...
    def call(self, inputs):
        """Return raw features. Use ``Qwen2Generate`` for logits / text."""
...
class Qwen2Generate(Qwen2Model):
    """Qwen2 with an LM head + greedy ``.generate()`` (text -> text)."""
