...
def dequantize_fp8(hf_state_dict):
    ...
    drop the scale keys; bf16 checkpoints pass through unchanged. The indexer's
    ``weights_proj`` is kept in fp32 in the checkpoint (no scale) and passes
    through."""
...
def drop_mtp_keys(hf_state_dict, num_layers):
    """Drop the multi-token-prediction layer (layer index ``num_layers``, ignored
    by transformers' GlmMoeDsaForCausalLM) and its private leaves."""
    out = {}
    for key, value in hf_state_dict.items():
...
def fuse_expert_weights(hf_state_dict):
    """Fuse per-expert ``mlp.experts.N.{gate,up,down}_proj`` (hub layout) into
    ``mlp.experts.gate_up_proj`` (E, 2I, H) / ``down_proj`` (E, H, I); fused
    in-memory state dicts pass through."""
    pat = re.compile(
...
def transfer_glm5_moe_weights(keras_model, hf_state_dict):
