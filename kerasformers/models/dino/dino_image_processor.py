...
class DinoImageProcessor(BaseImageProcessor):
    """Image processor for DINO (v1) ViT feature extractors.

    Matches transformers' ``ViTImageProcessor`` for ``facebook/dino-*``: a square
    resize to ``image_resolution`` (bilinear, through PIL on the raw uint8 image so
    the interpolation matches the reference and is backend-independent), rescale to
    ``[0, 1]``, and ImageNet-standard normalization. There is no center crop.
