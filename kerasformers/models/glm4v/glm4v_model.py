...
def glm_mrope_cos_sin(position_ids, rotary_dim, theta, mrope_section):
    """Merged M-RoPE tables for GLM-4V's partial interleaved rope.

    ``position_ids`` is ``(3, batch, seq)`` (temporal/height/width). Per-axis
    frequencies are computed over ``rotary_dim // 2`` channels, the three axes
    are collapsed by the ``mrope_section`` split (channel chunk ``i`` keeps axis
    ``i % 3``), then repeat-interleaved to ``rotary_dim``. Returns ``(cos, sin)``
    each ``(batch, seq, rotary_dim)``.
    """
    ...
    emb = ops.repeat(merged, 2, axis=-1)
    return ops.cos(emb), ops.sin(emb)


@keras.saving.register_keras_serializable(package="kerasformers")
class Glm4vTextModel(layers.Layer):
    """GLM-4V text decoder: ``embed -> num_layers x Glm4DecoderLayer -> RMSNorm``.

    Reuses the GLM-4 decoder block (sandwich norms, fused-SwiGLU MLP, biased
    q/k/v, partial interleaved rope); ``call`` consumes the multimodal-fused
    ``inputs_embeds`` plus the merged M-RoPE tables.
    """
...
class Glm4vModel(SubclassedBaseModel):
    ...
    Args:
        ...
        num_kv_heads: Text geometry.
        partial_rotary_factor: Fraction of each head rotated.
        norm_eps / rope_theta: Text norm/rope.
        mrope_section: Per-axis channel split of the merged M-RoPE.
        tie_embeddings: Whether the LM head ties to the token embedding.
        vision_*: Vision tower geometry.
        image_token_id / video_token_id / image_start_token_id /
        image_end_token_id / video_start_token_id / video_end_token_id: Specials.
    """

    HF_MODEL_TYPE = "glm4v"
    BASE_MODEL_CONFIG = GLM4V_CONFIG
    BASE_WEIGHT_CONFIG = GLM4V_WEIGHTS_URLS
    ...
    def config_from_hf(cls, hf_config):
        ...
        return {
            ...
            "image_token_id": hf_config.get("image_token_id", 151343),
            "video_token_id": hf_config.get("video_token_id", 151344),
    ...
    def transfer_from_hf(cls, keras_model, hf_state_dict):
        from .convert_glm4v_hf_to_keras import transfer_glm4v_weights

        transfer_glm4v_weights(keras_model, hf_state_dict)
