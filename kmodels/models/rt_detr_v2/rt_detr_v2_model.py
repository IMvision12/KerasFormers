...
def _rt_detr_v2_build(
    ...
    """Internal builder for RT-DETRv2 architecture (no per-layer class heads).

    Used by ``RTDetrV2Model``. The class heads live in ``RTDETRV2Detect``
    which composes ``RTDetrV2Model``.

    Returns:
        hs_last: Decoder ``last_hidden_state``.
...
class RTDetrV2Model(BaseModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        hs_last, last_boxes = _rt_detr_v2_build(
            img_input,
